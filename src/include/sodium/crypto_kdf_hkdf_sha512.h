...
int crypto_kdf_hkdf_sha512_extract(unsigned char prk[crypto_kdf_hkdf_sha512_KEYBYTES], const unsigned char *salt, size_t salt_len,
                                   const unsigned char *ikm, size_t ikm_len) __attribute__((nonnull(1)));

void crypto_kdf_hkdf_sha512_keygen(unsigned char prk[crypto_kdf_hkdf_sha512_KEYBYTES]) __attribute__((nonnull));

int crypto_kdf_hkdf_sha512_expand(unsigned char *out, size_t out_len, const char *ctx, size_t ctx_len,
                                  const unsigned char prk[crypto_kdf_hkdf_sha512_KEYBYTES]) __attribute__((nonnull(1)));
...
int crypto_kdf_hkdf_sha512_extract_init(crypto_kdf_hkdf_sha512_state *state, const unsigned char *salt, size_t salt_len) __attribute__((nonnull(1)));

int crypto_kdf_hkdf_sha512_extract_update(crypto_kdf_hkdf_sha512_state *state, const unsigned char *ikm, size_t ikm_len) __attribute__((nonnull));
