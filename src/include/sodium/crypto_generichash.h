...
int crypto_generichash_update(crypto_generichash_state *state, const unsigned char *in, unsigned long long inlen) __attribute__((nonnull(1)));

int crypto_generichash_final(crypto_generichash_state *state, unsigned char *out, const size_t outlen) __attribute__((nonnull));

void crypto_generichash_keygen(unsigned char k[crypto_generichash_KEYBYTES]) __attribute__((nonnull));
